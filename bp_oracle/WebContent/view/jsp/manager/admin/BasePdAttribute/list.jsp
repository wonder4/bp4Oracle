...
<div class="divContent">
  ...
  <html-el:form action="/admin/BasePdAttribute">
  ...
  <form id="listForm" name="listForm" method="post" action="BasePdAttribute.do?method=delete">
    <div style="text-align: left; padding: 5px;">
      ...
      <logic-el:match name="popedom" value="+1+">
        <input type="button" name="add" id="add" class="bgButton" value="添加" onclick="location.href='BasePdAttribute.do?method=add&mod_id=${af.map.mod_id}&attr_scope=${af.map.attr_scope }';" />
      </logic-el:match>
    </div>
    <table width="100%" border="0" cellpadding="1" cellspacing="1" class="tableClass">
      ...
      <c:forEach var="cur" items="${entityList}" varStatus="vs">
        <tr>
          ...
          <td align="left" nowrap="nowrap"><a title="查看" href="BasePdAttribute.do?method=view&amp;mod_id=${af.map.mod_id}&amp;id=${cur.id}">${fn:escapeXml(cur.attr_name )}</a></td>
          ...
          <td align="left"><c:forEach items="${cur.map.son_list}" var="son">
          ...
          <c:if test="${isContains}" >
            <td align="center" nowrap="nowrap"><logic-el:match name="popedom" value="+2+"> <a class="butbase" href="javascript:void(0);"><span class="icon-edit" onclick="confirmUpdate(null, 'BasePdAttribute.do', 'id=${cur.id}&' + $('#bottomPageForm').serialize())">修改</span></a> </logic-el:match>
              <logic-el:match name="popedom" value="+4+">
                ...
                <c:if test="${not isLock and not isDel}"> <a class="butbase" href="javascript:void(0);"><span class="icon-remove" onclick="confirmDelete(null, 'BasePdAttribute.do', 'id=${cur.id}&attr_scope=${af.map.attr_scope }&' + $('#bottomPageForm').serialize())">删除</span></a> </c:if>
              </logic-el:match></td>
          </c:if>
        </tr>
      </c:forEach>
  ...
  <div class="pageClass">
    <form id="bottomPageForm" name="bottomPageForm" method="post" action="BasePdAttribute.do">
