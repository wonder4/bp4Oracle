...
<div class="divContent">
  ...
  <html-el:form action="/admin/BasePdAttribute" enctype="multipart/form-data">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableClassTbody">
      <tr>
      ...
      <tr id="xz_hidden_tbody" >
        <td colspan="2"><table width="100%" border="0"  cellpadding="0" cellspacing="0"  align="left"  class="tableClassTbody">
            <tr>
...
$(document).ready(function(){
	...
	$("img[id='delSonAttr']").each(function(){
		$(this).click(function (){
			...
			$.ajax({
				type: "POST",
				url: "BasePdAttribute.do?method=checkDel",
				data: "id="+id,
				dataType: "json",
				error: function(request, settings) {alert("数据导出失败！");},
				success: function(returnDataArray) {
					if(returnDataArray.have_bm_record=='true'&&returnDataArray.have_pd_record!='true'){
						alert("有便民服务信息关联此属性，无法删除");
					}else if(returnDataArray.have_bm_record!='true'&&returnDataArray.have_pd_record=='true'){
						alert("有产品信息关联此属性，无法删除");
					}else if(returnDataArray.have_bm_record=='true'&&returnDataArray.have_pd_record=='true'){
						alert("有便民服务和产品信息关联此属性，无法删除");
					}else{
						if(confirm("确定删除？")){
...
function selectAllBrand() {
	var url = "BaseBrandInfo.do?method=listSelectAllBrand" + "&isSelectAll=true&azaz=" + Math.random() + "&time=" + new Date().getTime();
	asyncbox.open({
		...
		callback : function(action,iframe,returnValue){
			if (action == "ok" && returnValue != null) {
				...
				$("#xzShow").empty();
				for (var i = 0; i <= brandIdAndBrandNames.length-1; i++){
					if(null != brandIdAndBrandNames[i] && "" != brandIdAndBrandNames[i]){
