                ...
                <tr>
                  <td>待选择区
                    <html-el:select property="select1" multiple="true" style="width:290px;height:358px;" styleId="select1" onchange="moveSelected(this.form.select1, this.form.select2,'no');">
                      ...
                      <option value="${cur.cls_id}_${cur.cls_name}">${fn:escapeXml(cur.tree_name)}</option>
                      </c:if>
                      <c:if test="${empty af.map.isSelectAll}">
                          <c:if test="${cur.is_leaf eq 0}"><optgroup label="${fn:escapeXml(cur.tree_name)}"/></c:if>
                          <c:if test="${cur.is_leaf eq 1}"><option value="${cur.cls_id}_${cur.cls_name}">${fn:escapeXml(cur.tree_name)}</option></c:if>
                      </c:if>
                      </c:forEach>
                    </html-el:select></td>
                  <td>已选择区
...
function getQueryNames(key_word) {
   	...
   	$.ajax({
		...
		cache: false,
		url: "CsAjax.do?",
		data: "method=" + "getBaseClassList&ajax=1&pd_class_type=${af.map.pd_class_type}&" + "key_word=" + key_word,
		dataType: "json",
		...
		success: function(data) {
			if (data.list.length >= 1) {
				for(var i = 0; i < data.list.length; i++) {
					...
					var opt;
                    <c:if test="${not empty af.map.isSelectAll}">
                    opt = new Option(cur.tree_name,cur.cls_id+"_"+cur.cls_name);
                    $("#select1").get(0).add(opt);
                    </c:if>
                    <c:if test="${empty af.map.isSelectAll}">
    				if (cur.is_leaf == 0) {
						opt = document.createElement("optgroup");
						opt.label = cur.tree_name;
