...
<div class="divContent">
  ...
  <form id="listForm" name="listForm" method="post" action="Role.do?method=delete">
    <div style="text-align: left; padding: 5px;">
      ...
      <input type="button" name="add" id="add" class="bgButton" value="添加" onclick="location.href='Role.do?method=add&mod_id=${af.map.mod_id}';" />
    </div>
    <table width="100%" border="0" cellpadding="0" cellspacing="1" class="tableClass">
      ...
      <c:forEach var="cur" items="${roleList}">
        <tr>
          ...
          <td align="left"><a title="查看" href="Role.do?method=view&amp;mod_id=${af.map.mod_id}&amp;id=${cur.id}">${fn:escapeXml(cur.role_name)}</a></td>
          ...
          <td align="center"><a class="butbase" href="javascript:void(0);"><span class="icon-configure" onclick="doNeedMethod(null, 'ModPopedom.do', 'edit', 'mod_id=${af.map.mod_id}&amp;role_id=${cur.id}&amp;url=Role');">授权</span></a></td>
          <td align="center" nowrap="nowrap"><a class="butbase" href="javascript:void(0);"><span class="icon-edit" onclick="confirmUpdate(null, 'Role.do', 'id=${cur.id}&' + $('#bottomPageForm').serialize())">修改</span></a>
            ...
            <c:if test="${not isLock and not isDel}"> <a class="butbase" href="javascript:void(0);"><span class="icon-remove" onclick="confirmDelete(null, 'Role.do', 'id=${cur.id}&' + $('#bottomPageForm').serialize())">删除</span></a> </c:if></td>
        </tr>
      </c:forEach>
    </table>
  </form>
