...
<div class="divContent">
  ...
  <legend style="color: #F00;font-weight: bold;"><span style="color: #0066FF;font-weight: bold;">【${fn:escapeXml(baseClass.cls_name)}】</span>已授权的属性</legend>
  <c:if test="${empty baseAttributeLinkList}">
    <center>
      <font style="color:red;font-weight: bold;">暂无已授权属性，请在“未授权的属性”中选择</font>
    </center>
  </c:if>
  <c:if test="${not empty baseAttributeLinkList}">
    <table width="100%" border="0" cellpadding="1" cellspacing="1" class="tableClass">
      ...
      <c:forEach var="curLink" items="${baseAttributeLinkList}">
        <tr>
          <td style="text-align:left;">${fn:escapeXml(curLink.attr_name)} / ${fn:escapeXml(curLink.attr_show_name)}</td>
          <td><c:forEach var="curSonLink" items="${baseAttributeSonList}">
  ...
  <html-el:form action="/admin/BasePdClass">
    <div style="text-align: left; padding: 5px;">
      ...
      &nbsp;
      <html-el:button property="attribute_insert" value="添加属性并绑定" styleClass="bgButton" styleId="attribute_insert" onclick="location.href='BasePdAttribute.do?method=add&mod_id=${af.map.mod_id}&cls_id=${af.map.cls_id}&pd_class_type=${af.map.pd_class_type }&attr_scope=${af.map.attr_scope}';"/>
    ...
    <table width="100%" border="0" cellpadding="1" cellspacing="1" class="tableClass">
      ...
      <c:forEach var="cur" items="${baseAttributeList}">
        <tr>
          ...
          <td><c:forEach var="curSon" items="${baseAttributeSonList}">
