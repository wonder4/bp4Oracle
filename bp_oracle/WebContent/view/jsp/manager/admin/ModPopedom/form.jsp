    ...
    <table width="100%" border="0" cellpadding="1" cellspacing="1" class="tableClass">
      ...
      <c:forEach var="cur" items="${sysModuleList}">
        ...
        <c:if var="urlIsNull" test="${cur.mod_url eq null or empty cur.mod_url}">
          <tr>
            <td style="text-align:center;">${cur.level -1}</td>
            <td colspan="3" style="text-align:left;"><c:forEach begin="1" end="${cur.level -1}">&nbsp;</c:forEach>
        ...
        <tr>
          <td style="text-align:center;">${cur.level -1}</td>
          <td style="text-align:left;"><c:forEach begin="0" end="${cur.level -1}">&nbsp;</c:forEach>
            ${cur.mod_name}</td>
      ...
      <tr>
        <td colspan="4" align="center"><html-el:checkbox property="all" onclick="checkAll(this)"> 选择全部 </html-el:checkbox>
          <html-el:submit value="确 定" styleClass="bgButton" />&nbsp;
          <html-el:button property="back" styleClass="bgButton" value="返 回" onclick="history.back();" /></td>
      </tr>
    </table>
  </html-el:form>
</div>
