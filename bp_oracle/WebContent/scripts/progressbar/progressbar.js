...
USAGE:
	...
	$("#progressbar").progressBar(45, {showText: false, ctx:"${ctx}"});		// percentage with config
*/
(function($) {
	$.extend({
		progressBar: new function() {
			...
			this.defaults = {
				...
				boxImage	: '/scripts/progressbar/images/progressbar.gif',						// boxImage : image around the progress bar
				barImage	: {
								0:	'/scripts/progressbar/images/progressbg_red.gif',
								30: '/scripts/progressbar/images/progressbg_orange.gif',
								70: '/scripts/progressbar/images/progressbg_green.gif'
			...
			this.construct = function(arg1, arg2) {
				...
				return this.each(function(child) {
					...
					if (argpercentage != null && this.bar != null && this.config != null) {
						...
						bar.alt					= percentage + "%";
						bar.src					= config.ctx + config.boxImage;
						...
						if (typeof(config.barImage) == 'object') {
							for (var i in config.barImage) {
								if (percentage >= parseInt(i)) {
									image = config.ctx + config.barImage[i];
								} else { break; }
							}
						}
						return image;
