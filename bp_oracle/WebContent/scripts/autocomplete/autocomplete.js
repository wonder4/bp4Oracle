/*
 * @version 1.1 2009-09-27
 * @author 	Tomas,Kirda
 * @version 1.1.1 2010-02-03
 * @author 	Wu,Yang
 */

/*1.1.1 change log: add 'animate' ; add 'pageCount'; change 'get' to 'post' ; change 'text' to 'json' ; remove 'evel()'; add 'focus' */
/*1.1.2 change log: add 'theIdOfNeedClearElement';  changeDate:20110601*/
...
(function($) {
	...
	Autocomplete.prototype = {
		...
		initialize : function() {
			...
			$(
					'<div id="'
							...
							+ autocompleteElId
							+ '" style="display:none; width:50px;"></div></div></div>')
					.appendTo('body');
		...
		onValueChange : function() {
			...
			if("" != this.options.theIdOfNeedClearElement) {
				$("#" + this.options.theIdOfNeedClearElement).val("");
			}
		...
		getSuggestions : function(q) {
			...
			if (cr && $.isArray(cr.suggestions)) {
				...
				me.options.params.pageCount = me.options.pageCount;
		...
		processResponse : function(text) {
			try {
				if (!$.isArray(text.data)) {
					text.data = [];
				}
				this.cachedResponse[text.query] = text;
				if (text.suggestions.length === 0) {
					this.badQueries.push(text.query);
				}
				if (text.query === this.getQuery(this.currentValue)) {
