...
function createElementsToForm(file, serverData) {
	...
	if (divCreateElementsToForm) {
		...
		var fileName ='文件名称：<input id="' + fileId + '" value="' + file.name + '" type="text" class="fileNameClass" name="file_name" maxlength="100"></input>';
		var delElement = '<span class="delElement" onclick="deleteFile(\'' + entity_id + '\', \'' + fileId + '\', \'' + encodeURI(rel_url) + '\')" title="删除附件">&nbsp;删除</span>';
		var fileHiddenPath = '<input value="' + rel_url + '" type="hidden" name="file_path"></input>';
		
		var divWrapper = "<div>" + fileName + delElement + fileHiddenPath + "</div>";
		if (upload_single_file) {
			divCreateElementsToForm.empty();
		}
		divCreateElementsToForm.append(divWrapper);
		// $("#info").text(divCreateElementsToForm.html());
	}
}
