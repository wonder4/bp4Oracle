...
/**
 * @use : String[] rowLines = ExcelUtils.readExcelRowLines("文件路径", 0, 0); for (int i = 0; i < rowLines.length; i++) {
 *      logger.info("====rowLines======:{}", rowLines[i]); if (StringUtils.isNotBlank(rowLines[i])) { for (int i = 0; i
 *      < rowLines.length; i++) { String values[] = StringUtils.split(rowLines[i], ExcelUtils.EXCEL_LINE_DELIMITER); } }
 *      }
 */
...
public class ExcelUtils {
	 ...
	 * @param curSheet excel的sheet index, 从0开始
	 * @param startRow 读取exce的 开始行数,从0开始
	 */
